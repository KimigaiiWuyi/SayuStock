...
| 空 DATA_PATH 当缓存 | Full suite：`test_offline_all_weather_png` 只数到 4 个 spark（加密合成） | `has_quote_cache` 要有 `*_single-stock*_data.json`；不够 skip。见 [§10.5.12](./10-cicd-and-dev-workflow.md) |
