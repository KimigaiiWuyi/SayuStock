    ...
    **末尾自动清理**（同 模拟盘清盘 逻辑）。
    ...
    block2, t2_d, p2_d, d2_d, push2_ok, _ = await _run_round(
        ...
        task=(
            ...
            f"Step 5: get_sector_heatmap() → 行业默认 ranked 全表；概念默认两端明细"
            f"（include_all 可覆盖）+ top_rise/top_fall\n"
            f"        从最强板块的 top_stocks 里挑 1~3 只作为候选\n"
            f"Step 5b: 可选 get_market_ranking(rank_by=amount|main_inflow|roe|…) "
            f"扫量额/资金/ROE/净利增速等——**仅线索，禁止榜一即买**；"
            f"增速/ROE 高≠好公司，数据可被调节\n"
            f"Step 6: get_latest_news(limit=8) → 财经新闻；若主线/政策/行业催化需细节，"
            f"再 web_search_tool 1～2 次（query 具体；禁止用网页数字当现价）\n"
            f"\n"
            ...
            f"        7.7 **事件/舆情（必做，get_latest_news 不能代替个股）**：\n"
            f"            web_search_tool(代码或简称 + 本轮关心的信息类型)；\n"
            f"            摘要不足再 web_fetch_tool；归纳正负向线索写入评分\n"
            f"\n"
            ...
            f"        技术≈40% / 基本面≈30% / 舆情事件≈15% / 波动调节；\n"
            f"        **禁止**未做 7.7 事件检索就强 buy。\n"
            f"        **reason 必须引用**：\n"
            f"        - 大盘环境（来自 Step 4）\n"
            f"        - 板块选择理由（来自 Step 5）\n"
